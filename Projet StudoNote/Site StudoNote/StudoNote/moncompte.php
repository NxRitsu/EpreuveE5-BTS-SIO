                        ...
                        <form action="./moncompte.php" method="post">
                            ...
                            <div>
                                ...
                                if (isset($_POST['majPassword'])) {
                                    ...
                                    $resultat = mysqli_query($mysqli, "SELECT password FROM etudiant WHERE identifiant = '$username'");
                                    $row = mysqli_fetch_assoc($resultat);
                                    $motDePasseActuel = $row['password'];

                                    if ($motDePasseActuel != $oldPwd) {
                                        echo "L'ancien mot de passe n'est pas le bon";
                                    } else {
                                        if ($newPwd == $confPwd) {
                                            // Mettre à jour le mot de passe
                                            $nouveauMotDePasse = hash('sha1', $newPwd);
                                            if (!mysqli_query($mysqli, "UPDATE etudiant SET password = '$nouveauMotDePasse' WHERE identifiant = '$username'")) {
                        ...
                        <form action="./moncompte.php" method="post">
                            ...
                            <div>
                                ...
                                if (isset($_POST['majUsername'])) {
                                    $requete = "SELECT * FROM etudiant WHERE identifiant = '" . $_POST['newUsername'] . "'";
                                    $resultat = mysqli_query($mysqli, $requete);
                                    if (mysqli_num_rows($resultat) > 0) {
                                        echo "L'identifiant existe déja, choisissez en un autre.";
                                    } else {
                                        if (!mysqli_query($mysqli, "UPDATE etudiant SET identifiant = '" . $_POST['newUsername'] . "' WHERE identifiant = '$username'")) {
                                            echo "Une erreur est survenue";
                                        } else {
                                            echo "Mise à jour effectuée";
                                            $_SESSION["identifiant"] = $_POST['newUsername'];
                                        }
                                    }
                                }
