        ...
        <div class="row">
            <div class="w-50">
                <div class="card-all mb-4 mt-4 shadow-lg text-center animate__animated animate__fadeInLeft animate__fast">
                    <div class="card-body moyenneGen">
                    <img class="card-img-top" src="Images/Grade.png" style="width: 650px;" alt="Calendar PNG">
                        <?php
                        // Préparation de la requête pour récupérer le nombre d'absences
                        $stmt = $bdd->prepare("SELECT AVG(resultat) AS moyenne FROM note WHERE idEtudiant ='$id'");

                        $stmt->execute();

                        // Récupération du nombre d'absences
                        $moyenneGen = $stmt->fetch(PDO::FETCH_ASSOC)["moyenne"];
                        ?>
                        <p class="moyenne-notes">Votre moyenne générale est : <strong> <?php echo $moyenneGen; ?> / 20 </strong></p>
                    </div>
                </div>
            </div>
            <div class="w-50">
                <div class="card-all mb-4 mt-4 shadow-lg text-center animate__animated animate__fadeInRight animate__fast" style="backdrop-filter: blur(15px);">
                    <div class="card-body scrollable">
                        <div class="absence">
                            ...
                            $sql = "SELECT matiere, resultat FROM note WHERE idEtudiant = '$id' ORDER BY matiere";
                            $result = $conn->query($sql);

                            // Affichage des données dans un tableau HTML
                            echo "<table>";
                            echo "<thead><tr><th>Matière</th><th>Note</th></tr></thead>";
                            echo "<tbody>";
                            while ($row = $result->fetch_assoc()) {
                                echo "<tr><td>" . $row["matiere"] . "</td><td>" . $row["resultat"] . "</td></tr>";
                            }
                            echo "</tbody>";
                            echo "</table>";
                            ?>
